  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const jobApplicationStats = await axios.get(`${Config.NEXT_PUBLIC_SERVER_HOST}/fetchJobApplicationStatistics`);
        const totalJobApplications = jobApplicationStats.data.totalJobApplications;

        // Fetch total rejected jobs from the second API
        const rejectedJobsStats = await axios.get(`${Config.NEXT_PUBLIC_SERVER_HOST}/fetchTotalRejectedJobs`);
        const totalRejections = rejectedJobsStats.data.totalRejections;

        // Fetch total shortlisted applicants from the third API
        const shortlistedApplicants = await axios.get(`${Config.NEXT_PUBLIC_SERVER_HOST}/getTotalShortlistedApplicants`);
